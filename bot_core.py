...
BOT_VERSION = "v3.1 Enhanced Edition"
...
def send_welcome(message):
    ...
    current_gate = get_user_gate_preference(user_id).capitalize() or DEFAULT_GATE.capitalize() 
...
def generate_code_command(message):
    ...
    try:
        ...
        if amount_str == 'premium' or amount_str == 'p': 
            amount = 0
        ...
        code_list = "\n".join(codes)
        premium_note = "(Premium Code - Grants Premium Access)" if is_premium_code else f"(Credits: {amount})" 

        bot.reply_to(message, f"✅ <b>Redeem Code Generated!</b>\n\n<code>{code_list}</code>\n\n{premium_note}\n\nDistribute this code for users to redeem credits or Premium access.", parse_mode="HTML") 

    except (ValueError, IndexError):
        bot.reply_to(message, "❌ <b>Usage Error: Credit Amount/Type Required</b>\n\nUsage: <code>/code amount</code> (amount as integer for credits, or 'premium'/'p' for premium code)", parse_mode="HTML") 
    except Exception as e:
